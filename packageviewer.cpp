...
void PackageViewer::showRemoveContextMenu(QPoint point)
{
    ...
    {
        // ── MULTI-FILE INJECT (modificado) ──────────────────────────────────
        QStringList paths = QFileDialog::getOpenFileNames(this, "Select File(s) to Inject", QtHelpers::DesktopLocation());
        if (paths.isEmpty())
            return;
        ...
        if (totalCount == 1)
        {
            GetPackagePath(items.at(0), &basePackagePath, true);
            basePackagePath += "\\";

            if (isFolder && items.at(0)->parent() == NULL)
                basePackagePath.append(items.at(0)->text(0) + "\\");
        }
        ...
        {
            QString packagePath = basePackagePath;
            ...
            {
                failedFiles.append(pathInfo.fileName() + " (" + QString::fromStdString(error) + ")");
            }
        }

        if (!failedFiles.isEmpty())
            QMessageBox::warning(this, "Inject Error", "The following files could not be injected:\n\n" + failedFiles.join("\n"));

        if (injectedCount > 0)
            statusBar->showMessage(QString::number(injectedCount) + " file(s) injected successfully", 3000);
        // ────────────────────────────────────────────────────────────────────
    }
